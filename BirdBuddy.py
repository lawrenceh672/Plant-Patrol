	...
	def processFrame(self, frame):
		...
		for c in cnts:
			...
			if(listIndex != -1): #-1 if theres no existing box close enough
				if self.TrackedList[listIndex].classification == "unknown":
					classy = TrackedList[listIndex].classify(img,0.75) #Classify the image, 75% sure
					TrackedList[listIndex].classification = classy
				if TrackedList[listIndex].classification == "background": #if its the background weed it out
					TrackedList[listIndex].active = False #its reverted back to background, stop tracking it.
					continue
									
				#Now update the currently being tracked object
				x1 = x
				x2 = TrackedList[listIndex].x
				w1 = w
				w2 = TrackedList[listIndex].w
				y1 = y
				y2 = TrackedList[listIndex].y
				h1 = h
				h2 = TrackedList[listIndex].h
				
				TrackedList[listIndex].x = min(x1,x2) #make the box as big as possible
				TrackedList[listIndex].y = min(y1,y2)
				TrackedList[listIndex].w = max(x1+w1,x2+w2) - min(x1,x2)
				TrackedList[listIndex].h = max(y1+h1,y2+h2) - min(y1,y2)
					
				TrackedList[listIndex].active = True
				TrackedList[listIndex].frame_end = self.frame_count
				TrackedList[listIndex].image = img					
				#good to know the path it took
				track =  (x,y,w,h)
				TrackedList[listIndex].xywh_track.append(track)
		...
		for o in TrackedList:
			if o.active == False:
				...
				o.image = img
				BirdBuddy.db.addTrackedObject(o, path)
				TrackedList.remove(o)
				
				#write the original full definition image
				cv2.imwrite(path+str(frame_count)+".png", original_frame)
		...
		if StatusChange == True: #something was added or removed
			if len(TrackedList) == 0: #so we dont write empty room until movement is detected
				rectangles.write("The Room is Empty\n")
				self.Take_Screen_Grab = True #and lets use this opportunity to update the screen
			else:
				rectangles.write("There are " + str(len(TrackedList))+ "objects being tracked\n")
				StatusChange = 0 #We have recored the change in status
...
<body>
	<div class="row">
		...
		for index, row in recs.iterrows():
			...
			#likely = BirdBuddy.db.getPossibles(path,image_data) #return a tuple with likelihoods for each listed possbility
			...
			#Finish the table of captured images and teaching probabilities
			htmloutput = htmloutput + """
				</SELECT>
				</td>
			"""
